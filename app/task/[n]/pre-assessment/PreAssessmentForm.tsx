...
export function PreAssessmentForm({ position }: { position: TaskPosition }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (Object.keys(answers).length < preAssessmentQuestions.length) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/assessment/${position}/pre`, {
        method: "POST",
