...
export function IdeaForm({
  ...
  const draftKey = `draft-initial-idea-${position}`;
  const autosaveStatus = useDraft(draftKey, `${title}\n${description}`);

  async function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!title.trim() || !description.trim()) {
      setError("Fikir başlığı ve açıklaması zorunludur.");
      return;
    }
    setSubmitting(true);
    setError(null);
    try {
      ...
      const res = await fetch(`/api/idea/${position}/initial`, { method: "POST", body: form });
  ...
  return (
    <form onSubmit={onSubmit} className="mt-8 space-y-5">
      <div className="flex items-center justify-between">
        <TaskTimer durationMinutes={durationMinutes} />
      ...
      <SketchUpload label="Eskiz veya Diyagram" file={sketch} onChange={setSketch} />
